...
if (!("tohidchat" in chat)) chat.tohidchat = false
...
if (settings.alwaysonline) {
    ...
    conn.sendPresenceUpdate('unavailable', m.chat);
	   ...
	   /* const specificGroup = '120363042880465739@g.us';
const allowedSender = '917849917350@s.whatsapp.net';
if (m.chat === specificGroup && m.sender !== allowedSender) {
	...
	//if (m.chat !== '120363042880465739@g.us') return
       // if (m.chat === '120363042880465739@g.us' && m.sender !== '917849917350@s.whatsapp.net') return;
...
if (
  ...
  if (m.text.match(/(tohid|a|b|c|d|e|f|g|h|i|j|k|l|m|n|o|p|q|r|s|t|u|v|w|x|y|z)/gi)) {
    ...
    this.sendMessage(m.chat, {
      react: {
        text: (m.sender === '917849917350@s.whatsapp.net') ? "👑" : pickRandom(emojis),
        key: m.key
      }
    });
  }
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '😻 𝗦𝘂𝗽𝗲𝗿 𝗧𝗼𝗵𝗶𝗱𝗕𝗼𝘁-𝗠𝗗 😻') :
...
"mediaType": 1, 
sourceUrl: 'https://github.com/Tohidkhan6332/TOHID-AI' }}}, { quoted: fkontak2 })
apii.data = ''
